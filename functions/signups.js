...
module.exports = {
    set: async function (type, name, channel, message, client) {
        return new Promise(async (resolve, reject) => {
            if (!message.guild.id) {
                client.messages.send(message.channel, 'This can only be used in a sign-up channel.', 240);
                resolve(false);
            }
            if (type === '+') {
                signValue = 'yes';
            ...
            if (raid.locked) {
                if (client.config.userId != message.author.id) {
                    message.author.send('This raid is locked -- sign-ups can no longer be modified.');
                    resolve(false);
                } else {
                    client.messages.send(message.channel, 'This raid is locked -- sign-ups can no longer be modified.', 240);
                    resolve(false);
                }
            }
            ...
            if (!client.set.validName(characterName)) {
                ...
                client.messages.errorMessage(message.channel, playerMessage, 240);
                resolve(false);
            }
