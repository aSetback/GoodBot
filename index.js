...
client.on("message", async message => {
	...
	if (config.ignoreList.indexOf(message.member.displayName) >= 0) {
		return false;
	}
	...
	if (command === "epgp" && isGain) {
		...
		epgpData = fs.readFileSync(config.epgpFile, 'utf8');
		...
		if (player.toLowerCase() == 'all') {
			...
			message.author.send("```\n" + epgpMessage + "\n```");
			message.channel.send("EPGP: Standings have been sent to you via dire maul.");
			return false;	
		}
